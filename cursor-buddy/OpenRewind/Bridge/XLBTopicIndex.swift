...
public actor XLBTopicIndex {
    ...
    /// Single source of truth for the xlb toggle.
    ///
    /// This used to read `openclicky.xlb.enabled` directly, which is NOT the
    /// key the Settings switch writes (`@AppStorage` binds
    /// `AppBundleConfiguration.userXLBEnabledDefaultsKey` =
    /// "openClickyXLBEnabled"). Flipping the switch therefore had no effect
    /// on the index, and `CompanionManager` read the orphan key with the
    /// opposite default. Route everything through the same accessor so the
    /// switch, the env override and the index agree.
    public func isEnabled() -> Bool {
        AppBundleConfiguration.xlbEnabled()
    }
...
private final class LibraryFileParser {
    ...
    private func processNestedPart(
        ...
        // Only THEN does the structural walker treat `Name(inner)` as a
        // nested subtopic. Items whose inner is a short/tagless payload
        // (e.g. `Cuiaba Brazil(youtube*map.google*earth.google)`) fall
        // through structural and get re-attached to the RECORD title by
        // the deep-index enrichment pass (`harvestAllNamedItems` /
        // Python's `_enrich_from_local_reader`). Without this gate the
        // structural pass double-parents them under the wrapping keyword
        // item, inflating gnodes/gedges by thousands.
        //
            ...
            if let close = innerRange {
                ...
                if innerEnd > innerStart {
                    ...
                    if wasNewSubtopic {
                        ...
                        // Still absorb aliases into the topics table for lookups.
        ...
        if !isGroup, ne > ns, let name = string(ns, ne) {
            if name.count >= 2, name.count <= 120,
               !name.hasPrefix("http"), !name.hasPrefix("//") {
                ...
                // `docker` external_ref target. Keep the tagNames guard off
                // for graphify parity. `emitEdge` still dedupes and the
                // topics table is written by the structural walker's
                // subtopic emitter, not from this function.
                for p in parts {
                    if p.count < 2 || p.count > 120 { continue }
                    if p.hasPrefix("http") || p.hasPrefix("//") { continue }
                    if p.caseInsensitiveCompare(parent) == .orderedSame { continue }
                    emitEdge(parent, p, kind: "searchin")
                    // Graphify: `parent --searchin--> external_ref`. Matches
                    // the searchin loop in `_enrich_from_local_reader`.
                    let libraryFile = library + "-library"
                    let parentGID = Self.gnodeID(parent)
    ...
    private func harvestTopLevelAliasTag(from start: Int, to end: Int, parent: String, count: inout Int) {
        ...
        while i + tag.count <= end {
            ...
            // Python requires ` alias:` (space then tag) OR start-of-desc.
            // At depth == 0 only.
            ...
            if depth == 0, atBoundary, matchesLowercase(tag, at: i) {
                ...
                // Value runs until the next top-level ` <letters>:` boundary
                // or end. Track paren depth to avoid stopping at a `foo:`
                // that lives inside a paren.
                var vDepth = 0
                var j = valueStart
                var valueEnd = end
                while j < end {
                    ...
                    if b == 0x28 { vDepth += 1 }
                        ...
                        // Look ahead: sequence of ASCII letters followed by `:`.
                        var k = j + 1
                        while k < end {
                            let c = bytes[k]
                ...
                if valueEnd > valueStart, let raw = string(valueStart, valueEnd) {
                    ...
                    for piece in raw.split(separator: ",") {
                        ...
                        // Reject if it looks like a fragment with a paren.
                        if alias.contains("(") || alias.contains(")") { continue }
                        // Topic table row (Python's LibraryIndex tracks this).
                        if bind(name: alias, kind: .alias, parent: parent, browse: ">\(parent)/", tagName: "alias") {
                            count += 1
                        }
                        // Graphify: topic --alias_of--> alias
                ...
                return  // Python's `break` after the first top-level match.
            }
            i += 1
        }
    }
